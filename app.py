...
# 🔥 傳入 debug_mode 參數，用來控制記憶體
def fetch_booking_bundle(h_in_code, h_in_label, d1, h_out_code, h_out_label, d4, d2_org, d2_dst, d2_date, d3_org, d3_dst, d3_date, cabin, strict_ci, debug_mode):
    ...
    for attempt in range(3):
        try:
            ...
            if res.status_code == 200:
                ...
                # 💣 記憶體排雷：除非開了 Debug，否則立刻將這包龐大的 raw_data 銷毀不回傳
                return_raw = raw_data if debug_mode else None
                return {"status": "success", "raw": return_raw, "offer": parsed_offer, "title": bundle_title}
...
st.warning("⚠️ 此工具專門鎖定 CI 外站四段聯程票。記憶體防護已啟動，支援大範圍掃描。")

c_toggles = st.columns(2)
with c_toggles[0]: strict_ci_toggle = st.checkbox("🔒 嚴格鎖定純華航 (CI) 航班", value=True)
with c_toggles[1]: debug_mode = st.checkbox("🛠️ 開啟 Debug 模式 (顯示原始 JSON)", value=False) 
...
if st.button("🚀 啟動 Booking.com 獵殺引擎", use_container_width=True):
    if len(d1_date_range) != 2 or len(d4_date_range) != 2:
        ...
        if len(tasks) > 2000:
            ...
            st.warning("⚠️ 無有效組合，請檢查日期先後順序。")
        else:
            msg = st.warning(f"🔥 記憶體防護已啟動。正在對 {len(tasks)} 組航班進行交叉掃描...")
            pb = st.progress(0)
            ...
            with ThreadPoolExecutor(max_workers=5) as exe:
                # 送出任務，並把 debug_mode 傳進去
                futures = {
                    exe.submit(fetch_booking_bundle, t[0], t[1], t[2], t[3], t[4], t[5], d2_org, d2_dst, d2_date, d3_org, d3_dst, d3_date, cabin_choice, strict_ci_toggle, debug_mode): t 
                    for t in tasks
                }
                
                for idx, f in enumerate(as_completed(futures)):
                    # 💣 記憶體排雷：一收到回應，立刻將任務從待辦字典中「拔除」，強制釋放記憶體！
                    task_info = futures.pop(f, None)
                    
                    try:
                        res = f.result()
                        if res["status"] == "success":
                            if debug_mode and len(raw_debug_data) < 3 and res.get("raw"):
                    ...
                    # 降低重繪頻率，保護前端 WebSocket 不當機
                    if (idx + 1) % 5 == 0 or (idx + 1) == len(tasks):
                        pb.progress((idx + 1) / len(tasks), text=f"掃描進度: {idx+1}/{len(tasks)} (已尋獲: {len(valid_results)} 組)")

            pb.empty()
            msg.empty()

            if error_count > 0:
                st.toast(f"⚠️ 掃描結束。有 {error_count} 組因連線問題未取得報價。")
            ...
            if valid_results:
                ...
                st.error("❌ 查無符合條件之票價。可能原因：艙等無位、無聯程報價或純華航過濾導致。")

            if debug_mode and raw_debug_data:
                st.markdown("---")
                st.subheader("🛠️ Debug 原始 JSON (僅顯示前 3 筆)")
                for idx, debug_json in enumerate(raw_debug_data):
                    with st.expander(f"檢視 Debug 資料 #{idx + 1}"):
                        st.json(debug_json)
