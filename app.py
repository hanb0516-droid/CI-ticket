...
# 2. UI 介面與動態連動
# ==========================================
if "d1_city" not in st.session_state: st.session_state.d1_city = [f"{c} ({n})" for c, n in CI_ASIAN_HUBS["港澳"].items()]
if "d4_city" not in st.session_state: st.session_state.d4_city = [f"{c} ({n})" for c, n in CI_ASIAN_HUBS["港澳"].items()]
def sync_d1(): st.session_state.d1_city = ALL_FORMATTED_CITIES if "全部" in st.session_state.d1_reg else [f"{c} ({n})" for r in st.session_state.d1_reg if r in CI_ASIAN_HUBS for c, n in CI_ASIAN_HUBS[r].items()]
...
if st.session_state.engine_running:
    ...
    if st.button("🚀 啟動【跨夜永不中斷】全亞洲地毯獵殺", use_container_width=True):
        ...
        if not tasks: st.warning("無效組合"); st.stop()
