...
def send_email_report(res_list):
    ...
    msg = MIMEMultipart()
    msg['From'], msg['To'], msg['Subject'] = SENDER, RECEIVER, f"✈️ 華航獵殺報告 - 捕獲 {len(res_list)} 組"
    msg.attach(MIMEText("地毯式掃描已完成，詳細清單請見附件。", 'plain'))
    att = MIMEBase('application', 'octet-stream')
    att.set_payload(txt_content.encode('utf-8'))
    encoders.encode_base64(att)
    att.add_header('Content-Disposition', f"attachment; filename=Flight_Hunter_Results.txt")
    msg.attach(att)
    try:
...
with st.container():
    ...
    # D1
    with c_r1:
        st.multiselect("D1 區域過濾 (選填)", list(CI_GLOBAL_HUBS.keys()), key="input_d1_reg")
    ...
    with c_r4:
        if st.session_state.input_sync_hubs:
            ...
            st.multiselect("D4 區域過濾 (選填)", list(CI_GLOBAL_HUBS.keys()), key="input_d4_reg")
    ...
    st.checkbox("📧 搜尋結束後寄送 Email 通知", value=True, key="input_email_on")
    ...
    if st.button("🚀 啟動獵殺掃描", use_container_width=True):
        ...
        for h1_raw, h4_raw in product(st.session_state.input_d1_hubs, d4_final_hubs):
            ...
            for d1, d4 in product(d1_dates, d4_dates):
                if d1 > st.session_state.input_d2_dt: conflicts.append(f"D1({d1}) 比 D2 晚"); continue
                if d4 < st.session_state.input_d3_dt: conflicts.append(f"D4({d4}) 比 D3 早"); continue
        ...
        if not tasks:
            st.error("⚠️ 無法生成任務！請檢查日期邏輯。")
            if conflicts: st.warning(f"偵測到衝突：{list(set(conflicts))[0]}")
        else:
...
if st.session_state.engine_running:
    ...
    BATCH = 15
    # 🛡️ 確保進度條不會除以零
    prog_val = min(curr / total, 1.0) if total > 0 else 0.0
    st.progress(prog_val, text=f"🔥 進度: {curr}/{total} | 捕獲: {len(st.session_state.valid_offers)}")
    ...
    with ThreadPoolExecutor(max_workers=5) as exe:
        ...
        for f in as_completed(futures):
            ...
            if res["status"] == "quota_exceeded": 
                st.error("🚨 API 額度用盡！")
                st.session_state.task_idx = total; break
...
if not st.session_state.engine_running and st.session_state.valid_offers:
    st.markdown("---")
    st.success(f"🎉 獵殺完畢！共捕獲 {len(st.session_state.valid_offers)} 組特價票。")
