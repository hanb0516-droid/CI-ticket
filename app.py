...
with c_dest1:
    out_dest = st.text_input("去程終點 (如 PRG)", value="PRG")
    date_out = st.date_input("去程日期", value=datetime(2026, 6, 11))
with c_dest2:
    in_origin = st.text_input("回程起點 (如 FRA)", value="FRA")
...
with c2: children = st.number_input("兒童 (如 Hayden)", value=0)
...
if st.button("🚀 啟動深度獵殺模式", use_container_width=True):
    ...
    st.write(f"📊 掃描報告：去程找到 {len(in_found)} 個潛力點，回程找到 {len(out_found)} 個潛力點。")

    if not in_found or not out_found:
        st.error("🚨 樣本不足！這代表 API 暫時拒絕連線。請稍等 5 秒後再按一次，或微調日期。")
    else:
        # 取最有潛力的前 4 個起點與終點組合
        top_in = [x[0] for x in sorted(in_found, key=lambda x: x[1])[:4]]
        top_out = [x[0] for x in sorted(out_found, key=lambda x: x[1])[:4]]
        
        msg.warning(f"🔥 第二階段：雷達已鎖定！正在精算 16 組【{cabin_choice}】100% 真實報價與艙等字母...")
        ...
        if results:
            if base_long_price > 0:
                ...
                st.warning("⚠️ 無法獲取直飛基準價，僅顯示四段總價排名。")
