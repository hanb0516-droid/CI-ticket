...
if st.button("🚀 啟動 Booking.com 獵殺引擎", use_container_width=True):
    if len(d1_date_range) != 2 or len(d4_date_range) != 2:
        ...
        if len(tasks) > 2000:
            st.error(f"🚨 組合數 ({len(tasks)} 組) 過多，請縮小範圍，建議控制在 1500 組以內。")
        elif not tasks:
            st.warning("⚠️ 無有效組合，請檢查日期先後順序。")
        else:
            msg = st.warning(f"🔥 系統穩定鎖定中。正在對 {len(tasks)} 組日期與航點進行交叉精算...")
            pb = st.progress(0)
            
            valid_results = []
            raw_debug_data = []
            error_count = 0  # 靜默記錄錯誤次數，不干擾畫面
            ...
            with ThreadPoolExecutor(max_workers=5) as exe:
                ...
                for idx, f in enumerate(as_completed(future_to_task)):
                    ...
                    if res["status"] == "success":
                        # 記憶體鎖：最多只存 3 筆 Debug 資料
                        if debug_mode and len(raw_debug_data) < 3: 
                    ...
                    # UI 降頻優化：每 5 組才更新一次進度條，避免 WebSocket 崩潰
                    if (idx + 1) % 5 == 0 or (idx + 1) == len(tasks):
                        pb.progress((idx + 1) / len(tasks), text=f"進度: {idx+1}/{len(tasks)} (已尋獲: {len(valid_results)} 組)")
            ...
            # 顯示錯誤統計
            if error_count > 0:
                st.toast(f"⚠️ 掃描結束。其中有 {error_count} 組因 API 限制或網路問題未成功取得報價。")
            ...
            if valid_results:
                ...
                st.error("❌ 查無符合條件之票價。可能原因：艙等無位、該路線無四段聯程報價，或嚴格過濾導致。")
